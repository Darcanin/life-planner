...
// import style from './GoalEditorForm.module.sass'
...
export interface IGoalEditorFormProps {
	title: string
	description: string
...
const GoalEditorForm = ({ goalId }: GoalEditorFormProps) => {
	...
	const onSubmit: SubmitHandler<IGoalEditorFormProps> = (data) => {
		const updatedGoal = {
			...startData,
			title: data.title,
	...
	return (
		<form className='modal-editor-form' onSubmit={handleSubmit(onSubmit)}>
			...
			<div className='footer'>
				...
				<Button className='form-button' type='reset' onClick={close}>
					Отменить
				</Button>
			</div>
		</form>
